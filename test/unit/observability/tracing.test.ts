...
const mocks = vi.hoisted(() => ({
  ...
  addSessionToSpan: vi.fn(),
  getObservabilityConfig: vi.fn()
}));

// Setup mock behaviors
mocks.mockSpan.spanContext.mockReturnValue({
  traceId: 'test-trace-id',
  spanId: 'test-span-id'
});
mocks.mockGetTracer.mockReturnValue(mocks.mockTracer);
...
vi.mock('../../../src/observability/session-correlation.js', () => ({
  addSessionToSpan: mocks.addSessionToSpan
}));
...
describe('Distributed Tracing', () => {
  ...
  beforeEach(() => {
    ...
    mocks.mockTracer.startActiveSpan.mockImplementation((_name, callback) => {
      return callback(mocks.mockSpan);
    });

    mocks.mockGetActiveSpan.mockReturnValue(mocks.mockSpan);
  });
  ...
  describe('createMCPSpan', () => {
    ...
    it('should create span for successful operation', async () => {
      ...
      expect(mocks.mockGetTracer).toHaveBeenCalledWith('mcp-server', '1.0.0');
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('test-operation', expect.any(Function));
    ...
    it('should add session context when provided', async () => {
      ...
      expect(mocks.addSessionToSpan).toHaveBeenCalledWith(mockSessionContext);
    });
    ...
    it('should add custom attributes', async () => {
      ...
      expect(mocks.mockSpan.setAttributes).toHaveBeenCalledWith(attributes);
      expect(mocks.mockSpan.setAttributes).toHaveBeenCalledWith({
        'mcp.component': 'server'
      });
    });
    ...
    it('should handle and rethrow errors', async () => {
      ...
      expect(mocks.mockSpan.setStatus).toHaveBeenCalledWith({
        code: 2, // ERROR
        message: 'Test error'
      });
      expect(mocks.mockSpan.end).toHaveBeenCalled();
    ...
    it('should handle non-Error exceptions', async () => {
      ...
      expect(mocks.mockSpan.recordException).toHaveBeenCalledWith('string error');
      expect(mocks.mockSpan.setStatus).toHaveBeenCalledWith({
        code: 2, // ERROR
  ...
  describe('createToolSpan', () => {
    it('should create span with tool-specific attributes', async () => {
      const operation = vi.fn().mockResolvedValue('tool-result');
      ...
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('tool.hello', expect.any(Function));
      expect(mocks.mockSpan.setAttributes).toHaveBeenCalledWith({
        'mcp.tool.name': 'hello',
    ...
    it('should handle tools without parameters', async () => {
      const operation = vi.fn().mockResolvedValue('no-params');
  ...
  describe('createLLMSpan', () => {
    it('should create span with LLM-specific attributes', async () => {
      const operation = vi.fn().mockResolvedValue('llm-response');
      const result = await createLLMSpan('claude', 'claude-3-haiku', operation, mockSessionContext);

      expect(result).toBe('llm-response');
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith(
    ...
    it('should work with different providers', async () => {
      ...
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('llm.openai.gpt-4', expect.any(Function));

      vi.clearAllMocks();

      await createLLMSpan('gemini', 'gemini-2.5-flash-lite', operation);
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('llm.gemini.gemini-2.5-flash-lite', expect.any(Function));
    });
  });

  describe('createOAuthSpan', () => {
    it('should create span with OAuth-specific attributes', async () => {
      const callback = vi.fn().mockResolvedValue('auth-result');
    ...
    it('should handle different OAuth operations', async () => {
      ...
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('oauth.github.callback', expect.any(Function));

      vi.clearAllMocks();

      await createOAuthSpan('microsoft', 'refresh', callback);
      expect(mocks.mockTracer.startActiveSpan).toHaveBeenCalledWith('oauth.microsoft.refresh', expect.any(Function));
    });
  });

  describe('createTransportSpan', () => {
    it('should create span with transport-specific attributes', async () => {
      const callback = vi.fn().mockResolvedValue('transport-result');
    ...
    it('should handle HTTP transport operations', async () => {
      const callback = vi.fn().mockResolvedValue('http-result');
  ...
  describe('Trace Context Utilities', () => {
    describe('getCurrentTraceId', () => {
      it('should return trace ID from active span', () => {
        ...
        expect(mocks.mockGetActiveSpan).toHaveBeenCalled();
      });

      it('should return undefined when no active span', () => {
        mocks.mockGetActiveSpan.mockReturnValueOnce(undefined);
        const traceId = getCurrentTraceId();
    ...
    describe('recordExceptionInCurrentSpan', () => {
      ...
      it('should do nothing when no active span', () => {
        ...
        expect(mocks.mockSpan.recordException).not.toHaveBeenCalled();
        expect(mocks.mockSpan.setStatus).not.toHaveBeenCalled();
      });
    });
  });
});