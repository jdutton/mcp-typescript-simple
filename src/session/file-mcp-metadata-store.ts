...
import {
  ...
  MCPSessionMetadata,
} from './mcp-session-metadata-store-interface.js';
...
export class FileMCPMetadataStore implements MCPSessionMetadataStore {
  ...
  private loadSync(): void {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        ...
        logger.error('Failed to load sessions from file', error);
        throw error;
      }
    }
  }
  ...
  private async doSave(): Promise<void> {
    try {
      ...
      logger.error('Failed to save sessions to file', error);
      throw error;
    }
  }
  ...
  async reload(): Promise<void> {
    try {
      ...
      logger.error('Failed to reload sessions from file', error);
      throw error;
    }
  }
