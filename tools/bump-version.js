#!/usr/bin/env node
/**
 * Version Bump Script for mcp-typescript-simple
 *
 ...
 *
 * Examples:
 *   node tools/bump-version.js 0.9.0
 *   node tools/bump-version.js 0.9.0-rc.1
...
Usage:
  ...
  npm run bump-version <version|increment>

Arguments:
  version      Explicit version (e.g., 0.9.0, 1.0.0, 0.9.0-rc.1)
  increment    patch, minor, or major (auto-calculates from current version)

Examples:
  node tools/bump-version.js 0.9.0           # Set to explicit version
  node tools/bump-version.js 0.9.0-rc.1      # Set to release candidate
  node tools/bump-version.js patch           # Increment patch (0.9.0 -> 0.9.1)
  node tools/bump-version.js minor           # Increment minor (0.9.0 -> 0.10.0)
  node tools/bump-version.js major           # Increment major (0.9.0 -> 1.0.0)
...
function incrementVersion(currentVersion, type) {
  // Strip any prerelease suffix for increment calculation
...
if (['patch', 'minor', 'major'].includes(versionArg)) {
  // Get current version from example-mcp package (primary package)
  try {
    const mcpPkgPath = join(PROJECT_ROOT, 'packages/example-mcp/package.json');
    const mcpPkg = JSON.parse(readFileSync(mcpPkgPath, 'utf8'));
    const currentVersion = mcpPkg.version;

    if (!currentVersion) {
      log('✗ Could not determine current version from @mcp-typescript-simple/example-mcp', 'red');
      process.exit(1);
    }
  ...
  if (!/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(newVersion)) {
    ...
    log('  Examples: 0.9.0, 1.0.0, 0.9.0-rc.1, 1.0.0-beta.1, patch, minor, major', 'yellow');
    process.exit(1);
  }
}
...
function updatePackageVersion(filePath, newVersion, skipPrivate = true) {
  try {
    ...
    // For root package.json, always update even if private
    const isRootPackage = filePath.endsWith(join('mcp-typescript-simple', 'package.json'));

    // Skip private packages unless root
...
try {
  ...
  if (result.skipped) {
    log(`  - ${result.name || 'mcp-typescript-simple'}: skipped (${result.reason})`, 'yellow');
  } else if (result.updated) {
    log(`  ✓ ${result.name || 'mcp-typescript-simple'}: ${result.oldVersion} → ${result.newVersion}`, 'green');
  } else {
    log(`  - ${result.name || 'mcp-typescript-simple'}: already at ${result.newVersion}`, 'yellow');
  }
} catch (error) {
...
if (skippedCount > 0) {
  log(`   Skipped: ${skippedCount} (private packages or already at ${newVersion})`, 'yellow');
}
console.log('');
console.log('Next steps:');
console.log(`  1. Review changes: git diff`);
console.log(`  2. Build packages: npm run build`);
console.log(`  3. Commit: git add -A && git commit -m "chore: Bump version to v${newVersion}"`);
console.log(`  4. Tag: git tag v${newVersion}`);
console.log(`  5. Push: git push origin main && git push origin v${newVersion}`);
console.log('');
console.log('Or use automated release script:');
console.log(`  npm run release:rc   # For release candidates`);
