...
echo "0/3: Cleaning stale build info..."
...
echo "1/3: Building base packages (tools, tools-llm)..."
...
echo "2/3: Building server package..."
npm run build -w @mcp-typescript-simple/server --if-present

# Build dependent packages (depend on base packages)
echo "3/3: Building example packages (example-tools-basic, example-tools-llm)..."
